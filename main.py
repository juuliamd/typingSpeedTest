...
def draw_board(x, st, start=0, end=0):
    ...
    words_per_row = 8  # Adjust this value to change the number of words per row
    for i in range(line, min(line + words_per_row, len(x))):  # Adjust the range for multiple words
        word = x[i]
        text = text_font.render(word, 1, text_colour)
        text.set_alpha(125)
        win.blit(text, (x_t, y_t))
        x_t += text.get_width() + 10  # Add some spacing between words
...
def main():
    global count, line, correct_st, cur_move, x_cur, theme
    ...
    #theme_change = pg.Rect(0, 0, 20, 20)
    while run:
...
def sequence():  # to make sequence of different length of the words
    li = []
    while sum(li) < 50:
...
def word_seq(li):  # to select words from the file with same length as given in the sequence
    st = ''
...
def verify(a, l):
    ...
    if alphabet(a) == l[line][count]:
        ...
        # Do not move the cursor if the entered letter is incorrect
        cur_move = False
